...
그래서 인증 방식은 아래 요구사항을 만족하도록 고려.

### - 요구사항: Scale-out 환경에서 로그인 상태가 안정적으로 유지되어야 함
트래픽이 커지면서 서버의 수평 확장(Scale-out)을 생각하게 되었지만 세션 방식을 사용시 세션을 서버 메모리에 저장하면, 사용자가 '서버1'에서 로그인한 뒤 로드밸런싱으로 '서버2'로 라우팅될 경우 세션을 찾지 못해 로그인이 풀릴 수 있음. 
